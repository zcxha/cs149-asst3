...
/// <summary>
/// if input[i] == input[i+1], then flag[i] = 1
/// </summary>
/// <param name="input"></param>
/// <param name="length"></param>
/// <param name="output"></param>
/// <returns></returns>
__global__ void flag_kernel(int* input, int length, int* output) {
...
/// <summary>
/// output as index, scatter input to output by output
/// 
/// </summary>
/// <param name="input"></param>
...
/// <returns></returns>
__global__ void scatter_kernel(int* flag, int* index, int* output, int length) {
	int idx = blockDim.x * blockIdx.x + threadIdx.x;
	if (idx < length - 1) {
		if (flag[idx]) {
...
/*
1 2 2 3 4 5 5 6 origin

0 1 0 0 0 1 0 0 flag

0 0 1 1 1 1 2 2 exclusive scan
...
int find_repeats(int* device_input, int length, int* device_output) {
	int num_blk = (length + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK;
	...
	flag_kernel <<<num_blk, THREADS_PER_BLOCK >>> (device_input, length, device_flag);
	cudaDeviceSynchronize();
	cudaMemcpy(device_scan, device_flag, length * sizeof(int), cudaMemcpyDeviceToDevice);
	exclusive_scan(nullptr, length, device_scan);
	cudaDeviceSynchronize();
	// get count
	int count = 0;
	cudaMemcpy(&count, device_scan + length - 1, sizeof(int), cudaMemcpyDeviceToHost);

	scatter_kernel << <num_blk, THREADS_PER_BLOCK >> > (device_flag, device_scan, device_output, length);
	return count;
}
